...
class AddFade(nn.Module):
    def __init__(self, max_fade_size=0.5, fade_shape=None, fix_fade_size=False):
    ...
    def add_fade(self, audio, fade_in_len, fade_out_len, fade_shape):
        fade_transform = torchaudio.transforms.Fade(
            fade_in_len=fade_in_len, fade_out_len=fade_out_len, fade_shape=fade_shape
        )
...
class WaveTimeStretch(nn.Module):
    ...
    def time_strech(self, audio, max_ratio, min_ratio, n_fft):
        ...
        return TorchTimeStretch.time_stretch(
            input=audio,
...
class CodecApply(nn.Module):
    ...
    def codec_apply(self, audio, codec):
        ...
        return torchaudio.functional.apply_codec(
            waveform=audio, sample_rate=self.sample_rate, format="wav", encoding=codec
        ).squeeze(0)
...
class AddEnvironmentalNoise(nn.Module):
    def __init__(
        ...
        if self.noise_path == None:
            if self.noise_category == None:
                ...
                for noise_category in self.noise_filename_dict.keys():
                    noise_path = os.path.join(
                        self.noise_dataset_path,
                        self.noise_filename_dict[noise_category],
                    )
                    noise_tensor, noise_tensor_power = self.load_noise_and_power(
                        noise_path
                    )
                ...
                self.noise_path = os.path.join(
                    self.noise_dataset_path,
                    self.noise_filename_dict[self.noise_category],
                )
                self.noise_tensor, self.noise_tensor_power = self.load_noise_and_power(
                    self.noise_path
                )
        else:
            self.noise_tensor, self.noise_tensor_power = self.load_noise_and_power(
            ...
            print(
                f"Add Environmental Noise Augmentation Initialized. Noise path: {self.noise_path}, Noise shape {self.noise_tensor.shape}, Noise power {self.noise_tensor_power}"
            )
    ...
    def load_noise_and_power(self, noise_path):
        ...
        noise_tensor = torchaudio.functional.resample(
            noise_tensor, noise_sr, self.sample_rate
        )  # The ESC-50 dataset have a 44.1kHz sr.
        if noise_tensor.shape[-1] < self.audio_len:
            noise_tensor = noise_tensor.repeat(
                self.audio_len // noise_tensor.shape[-1] + 1
            )[: self.audio_len]
        else:
...
class ResampleAugmentation(nn.Module):
    ...
    def __init__(self, resample_rate: list, original_sr: int = 16000, device="cuda"):
        ...
        for resample_rate in self.resample_rate:
            self.resample_transforms.append(
                torchaudio.transforms.Resample(
                    orig_freq=self.original_sr, new_freq=resample_rate
                ).to(device)
            )
...
class MoCoAudioDataset(torch.utils.data.Dataset):
    ...
    def pad_or_clip(self, audio):
        ...
        if audio.shape[-1] < self.audio_len:
            audio = audio.repeat(self.audio_len // audio.shape[-1] + 1)[
                : self.audio_len
            ]
    ...
    def __getitem__(self, index):
        ...
        audio = torchaudio.load(self.root_dir + key)[
            0
        ]  # torchaudio.load returns a tuple (tensor, sample_rate)
        ...
        key = key[
            :-5
        ]  # since the key have been added the .flac suffix, we remove it here by take out the last 5 chars
...
class Dataset_ASVspoof2019_train(Dataset):
    ...
    def __getitem__(self, index):
        ...
        x_inp = torch.unsqueeze(
            x_inp, 0
        )  # added by haulyn5, add a dimension for channels In order to be consistent with the previous dataset
        y = self.labels[key]
